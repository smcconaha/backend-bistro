...
def testing(request):
    menus = list(Menu_item.objects.values()) # wrap in list(), because QuerySet is not JSON serializable.  Values is QS of dicts
    pprint(menus)
    return JsonResponse({'data': menus})

def menu(request):
    data = [i.menu_convert() for i in Menu_item.objects.all()]
